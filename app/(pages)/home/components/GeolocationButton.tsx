...
const GeoLocationButton: React.FC = () => {
  ...
  const dispatchSignupLatitude = (e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch(setLatitude(Number(e.target.value)));
  };

  const dispatchSignupLongitude = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div className="flex max-h-12">
      ...
      <input
        ...
        onChange={dispatchSignupLatitude}
      />
      위도
      <input
        type="number"
        className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
        onChange={dispatchSignupLongitude}
      />
      경도
    </div>
  );
